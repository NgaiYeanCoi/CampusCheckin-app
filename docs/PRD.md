...
| 教师首页 | 教师 | 展示教师课程和管理入口 | 课程列表、发起签到入口 | 是 |
| 教师发起签到页 | 教师 | 创建签到任务 | 课程选择、签到时间、课堂口令 | 是 |
...
- 遵守 `docs/notion/DESIGN.md` 的视觉方向，并适配校园考勤场景。
...
- API 统一返回 `ApiResponse { code, message, data }`。
- Android 模拟器默认 baseUrl 为 `http://localhost:8080/api/v1/`。
